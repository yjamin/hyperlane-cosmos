...
func (x *fastReflection_QueryIsmsResponse) ProtoMethods() *protoiface.Methods {
	...
	marshal := func(input protoiface.MarshalInput) (protoiface.MarshalOutput, error) {
		...
		if len(x.Ism) > 0 {
			for iNdEx := len(x.Ism) - 1; iNdEx >= 0; iNdEx-- {
				encoded, err := options.Marshal(x.Ism[iNdEx])
				if err != nil {
					return protoiface.MarshalOutput{
						NoUnkeyedLiterals: input.NoUnkeyedLiterals,
						Buf:               input.Buf,
...
var file_hyperlane_ism_v1_query_proto_rawDesc = []byte{
	...
	0x69, 0x73, 0x6d, 0x2f, 0x76, 0x31, 0x2f, 0x70, 0x61, 0x72, 0x61, 0x6d, 0x73, 0x42, 0xcc, 0x01,
